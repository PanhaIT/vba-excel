Attribute VB_Name = "save_pdf_print_adj"

Public  Sub savePdfInvAdjustment()
    OnStart
    ...
    confirmBoxAlert      = MsgBoxW("Are you want to export print inventory adjustment as PDF", vbYesNoCancel  +  vbQuestion + vbDefaultButton1, "Export PDF")
    Set activeWorkbook   = Workbooks("index.xlsm")
    Set ws_file_setting  = activeWorkbook.Sheets("FilesSetting")
    Set ws_print_a4      = activeWorkbook.Sheets("PrintInvAdjustment")
    ...
    directoryInvoice     = Application.Vlookup(5,rng_file_setting,10,FALSE)
    invoiceCode          = ws_print_a4.Range("print_adj_code").Value
    
    If confirmBoxAlert = vbYes And checkDatabaseFile = TRUE Then
        If Len(Application.Vlookup(5,rng_file_setting,10,FALSE)) > 0 Then
            directoryInvoice = Left(directoryInvoice, Len(directoryInvoice) - 1)
            If MyFSO.FolderExists(directoryInvoice) = True Then
                pdfFileName  = invoiceCode & "_" & Format(ws_print_a4.Range("print_adj_date").Value, "dd-mm-yyyy") & ".pdf"
                fileExist    = checkFileIsExist(CStr(pdfFileName), CStr(directoryInvoice))
                If fileExist = True Then 
                    MsgBox "File inventory adjustment (" & CStr(pdfFileName) & ") already saved."
                    Exit Sub
                Else
...
Public  Sub getExchangeRate()
    ActiveSheet.Range("print_adj_exchange_rate").Value = Sheets("NBC_Exchange_Rate").Range("ExchangeRateToday").Value
End Sub
